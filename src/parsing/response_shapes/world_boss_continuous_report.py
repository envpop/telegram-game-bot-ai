...
只驗證過 2 則實際樣本：一般格式(7 刀，含標頭跟逐刀清單)、跟換相後恢復
攻擊剛好把王打死的精簡格式(沒有標頭、沒有逐刀清單，只剩「今日討伐次數
用完(X/Y)⚔️ 王被殺後下一階現身...」這句)。後者這種情況 boss_name/
hit_damages/total_hits/total_damage 都會是 None，呼叫端(furnace_loop_
strategy.py)只在乎 daily_count/daily_limit，兩種格式都抓得到，不受影響。

還沒看到：中途王被打死、次數沒用完就停止的版本(「今日討伐次數用完」這句
話可能不會出現，用什麼代替不知道)。之後補到新樣本再回頭驗證，目前
daily_count/daily_limit 抓不到時就是 None，呼叫端要自己處理「不知道」
的情況，不要假設一定抓得到。
...
def signature(text):
    # 2026-09-06 擴大：換相後恢復攻擊、剛好在這次「連續討伐」裡把王打死
    # 的情況，回覆會整段精簡成只剩結尾這句，沒有「⚔️⚡【連續討伐】世界王」
    # 標頭、也沒有逐刀清單——只要有「今日討伐次數用完(X/Y)」這句就認得，
    # 不強求標頭一定要在。
    return "【連續討伐】世界王" in text or bool(RE_DAILY_EXHAUSTED.search(text))
